...
class APILiteLLM(LanguageModel):
    ...
    def __init__(self, model_name):
        ...
        self.is_openrouter = self.model_name in OPENROUTER_MODEL_NAMES
    ...
    def get_litellm_model_name(self, model_name):
        if model_name in TOGETHER_MODEL_NAMES:
            ...
            litellm_name = OPENROUTER_MODEL_NAMES[model_name]
        ...
        if self.is_openrouter:
            ...
            def generate_one(messages):
                for attempt in range(1, 4):
                    try:
                        logger.info(
                            f"Calling attacker {self.litellm_model_name} "
                            f"(attempt {attempt}/3)."
                        )
                        completion = client.chat.completions.create(
                            model=self.litellm_model_name.removeprefix("openrouter/"),
                            messages=messages,
                            temperature=temperature,
                            top_p=top_p,
                            max_tokens=max_n_tokens,
                            seed=0,
                            stop=eos_tokens or None,
                            extra_body={
                                "reasoning": {
                                    "enabled": True,
                                    "exclude": True,
                        ...
                        if not output:
                            raise RuntimeError("Attacker returned an empty response.")
                        return output
                    except (
                        APIConnectionError,
                        APITimeoutError,
                        ...
                        logger.warning(
                            f"Transient attacker API failure; retrying in "
                            f"{wait_seconds} seconds."
                        )
                        time.sleep(wait_seconds)

            with ThreadPoolExecutor(max_workers=len(convs_list)) as executor:
                return list(executor.map(generate_one, convs_list))
